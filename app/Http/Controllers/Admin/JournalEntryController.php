...
class JournalEntryController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $journal_items = JournalItem::where('journal_id', $id)->get();

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted', 'journal_items'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $accounts = ChartOfAccount::where('is_enabled',1)->get();
        $journal_items = JournalItem::where('journal_id',$id)->get();

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'accounts','journal_items'));
    }
    ...
    public function update(Request $request, $id)
    {
        if(array_sum($request->debit) != array_sum($request->credit)){
            return redirect()->back()->with([
                'message'    => "Debit and Credit Doesn't Match",
                'alert-type' => 'error',
            ]);
        }
        ...
        try {
            ...
            $journal->descriptions = $request->descriptions;
            $journal->save();

            $this->destroyJournalItem($id);
            foreach($request->account as $i => $value){
                $journal_item = new JournalItem;
                $journal_item->journal_id = $journal->id;
                $journal_item->account = $request->account[$i];
                $journal_item->debit = $request->debit[$i];
                $journal_item->credit = $request->credit[$i];
                $journal_item->description = $request->description[$i];
                $journal_item->save();
            }
            DB::commit();
        } catch (\Throwable $th) {
        ...
        return redirect('admin/journal-entries')->with([
    ...
    public function create(Request $request)
    {
        ...
        $accounts = ChartOfAccount::where('is_enabled',1)->get();
        $c_journal = setting('system-setting.journal_prefix').str_pad(JournalEntry::get()->count()+1, 5, '0', STR_PAD_LEFT);

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'accounts','c_journal'));
    }
    ...
    public function destroyJournalItem($id)
    {
