...
class InvoicePaymentController extends Controller
{
    ...
    {
        //add currency
        $request['currency'] = setting('system-setting.currency');
        ...
        $transaction_stores = [
            "user_id" => Invoice::find($request->invoice_id)->customer_id,
            "user_type" => "Customer",
        ...
        if (!$request->has('_tagging')) {
            if (auth()->user()->can('browse', $data)) {
                $redirect = redirect('admin/invoices/'.$request->invoice_id);
            } else {
                $redirect = redirect()->back();
            }
